...
export async function GET(request: Request) {
  try {
    ...
    const source = searchParams.get('source') || 'ALL'; // 'ALL' | 'EVENT' | 'OPPORTUNITY' | 'EXAM' | 'TEAM'
    ...
    if (source === 'ALL' || source === 'EVENT') {
      try {
        ...
        if (eventId && eventId !== 'ALL') {
          registrations = registrations.filter((r: any) => r.eventId === eventId);
        }
    ...
    if (source === 'ALL' || source === 'OPPORTUNITY') {
      try {
        ...
        for (const app of applications) {
          ...
          if (email && email.includes('@') && !recipientMap.has(email)) {
            recipientMap.set(email, {
              email,
              name: app.name || 'Opportunity Applicant',
