  ...
  try {
      ...
      return NextResponse.json(
        {
          ...
          candidate: {
            ...
            qualityTier: signaling?.qualityTier || 'GOOD',
            networkStats: signaling?.networkStats || null,
            actualCameraSettings: signaling?.actualCameraSettings || null,
            reconnectCount: signaling?.reconnectCount || 0,
    ...
    const enrichedAttempts = filteredAttempts.map(att => {
      ...
      return {
        ...
        qualityTier: signaling?.qualityTier || (att.cameraStatus === 'DISCONNECTED' ? 'DISCONNECTED' : 'GOOD'),
        networkStats: signaling?.networkStats || null,
        actualCameraSettings: signaling?.actualCameraSettings || null,
        reconnectCount: signaling?.reconnectCount || 0,
