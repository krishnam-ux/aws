...
export async function POST(request: Request) {
  try {
    ...
    if (attempt.status === 'LOCKED') {
      return NextResponse.json({ error: 'Exam is currently locked. Please wait for the proctor to verify and unlock your attempt.' }, { status: 403 });
    }

    if (attempt.status === 'VERIFIED') {
      return NextResponse.json({ error: 'Identity verified. Please wait for the proctor to unlock the exam session.' }, { status: 403 });
    }
