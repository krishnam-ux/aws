    ...
    <main className="bg-white">
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16 sm:py-20">
        <div className="max-w-3xl mb-10">
          <p className="text-xs font-bold uppercase tracking-[0.2em] text-aws-orange mb-4">Opportunities</p>
          <h1 className="font-display font-black text-3xl sm:text-4xl lg:text-5xl text-brand-navy tracking-tight mb-4">Opportunities</h1>
          <p className="text-base sm:text-lg text-slate-600 leading-relaxed">
            Discover student leadership roles, founding community opportunities, internships, jobs, volunteering experiences, and campus initiatives that help you build your future.
          </p>
        </div>
        ...
        {publicOpportunities.length === 0 ? (
          <div className="rounded-xl border border-slate-200 bg-slate-50 p-10 text-center">
            ...
            <p className="text-slate-600 mt-2">Please check back soon for new roles, internships, and community opportunities.</p>
          </div>
        ) : (
                ...
                <article key={career.id} className="tech-card-new tech-card-new-hover p-5 flex flex-col h-full">
                  ...
                  <div className="mt-auto pt-4 border-t border-slate-200">
                    <div className="flex items-center justify-between text-xs text-slate-500 mb-4">
                      <span>Posted: {new Date(career.createdAt || Date.now()).toLocaleDateString('en-IN')}</span>
                      <span>{career.status}</span>
                    </div>
                    <Link href={`/opportunities/${career.slug}`} className="btn-primary w-full justify-center text-center">View Opportunity</Link>
                  </div>
                </article>
              );
            })}
