...
// Check for the 2-column signature layout code
const checks = {
  '2-column alignment': content.includes('const usableWidth = pageWidth - margin * 2;') && content.includes('const columnWidth = usableWidth / 2;'),
  'leftSignX calculation': content.includes('const leftSignX = margin + (columnWidth / 2) - (signatureLineWidth / 2);'),
  'rightSignX calculation': content.includes('const rightSignX = margin + columnWidth + (columnWidth / 2) - (signatureLineWidth / 2);'),
  'both lines at same height': 
  ...
  'centered names under signatures': content.includes("{ align: 'center' }") && content.includes('Abhay Shukla'),
